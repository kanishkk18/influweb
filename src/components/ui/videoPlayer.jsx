  ...
  const getEmbedUrl = (url, platform) => {
    try {
      if (platform === 'youtube') {
        ...
        // Updated regex to handle both /p/ (posts) and /reel/ (Reels)
        const instagramRegex = /instagram\.com\/(?:p|reel)\/([^/?]+)/;
        const match = url.match(instagramRegex);
        if (match && match[1]) {
          // Instagram embed URLs work with /p/ for both posts and Reels
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Button 
                    ...
                    size="icon" 
                    className="h-8 w-8 px-6  bg-red-500 text-white hover:bg-white/20"
                    onClick={onClose}
                  >
close 
                  </Button>
