...
import { Youtube, Eye, Users, Star, TrendingUp } from "lucide-react";
import { formatNumber } from "@/utils/mockData";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";

export function YouTubeInfluencerCard({ influencer }) {
  ...
  return (
    <Card className="overflow-hidden transition-all border-0 shadow-xl bg-gradient-to-br from-gray-800/90 to-gray-900/90 backdrop-blur-md text-white">
      <CardHeader className="relative p-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-red-600/80 to-red-700/80 z-0"></div>
        
        {/* Decorative elements */}
        <div className="absolute -bottom-3 right-6 w-20 h-20 bg-red-500/30 rounded-full blur-2xl"></div>
        <div className="absolute top-2 right-2 w-10 h-10 bg-red-500/40 rounded-full blur-xl"></div>
        
        <div className="relative z-10 flex items-center gap-4 p-5">
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-br from-red-500 to-red-700 rounded-full blur-sm -z-10"></div>
            <Avatar className="h-20 w-20 border-2 border-white/30 ring-2 ring-red-500/30 shadow-lg">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center text-white/90 text-sm mt-1">
              <Youtube className="h-3.5 w-3.5 mr-1 text-red-400" />
              <span className="text-gradient bg-gradient-to-r from-red-400 to-red-500 bg-clip-text text-transparent">
                {influencer.username}
              </span>
            </div>
          </div>
      ...
      <CardContent className="p-5">
        <div className="grid grid-cols-2 gap-1 mb-5">
          <div className="text-center p-2 bg-gray-800/50 rounded-lg backdrop-blur-sm">
            ...
            <p className="font-bold text-white">{formatNumber(influencer.subscribers)}</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <Badge variant="outline" className="bg-red-500/10 border-red-400/30 text-red-400">
            {influencer.niche}
          </Badge>
          <Badge variant="outline" className="bg-orange-500/10 border-orange-400/30 text-orange-400">
            {influencer.category}
          </Badge>
