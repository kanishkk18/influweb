...
// Fetch all candidates action
export async function fetchAllCandidatesAction() {
  await connectToDB();
  
  // Find all profiles with role 'candidate'
  const candidates = await Profile.find({ role: 'candidate' });
  
  // For each candidate, fetch their applications
  ...
  return JSON.parse(JSON.stringify(candidatesWithData));
}

export async function fetchAllCandidatesWithDetailsAction() {
  await connectToDB();
    ...
    candidates.map(async (candidate) => {
      ...
      const applicationsWithJobs = await Promise.all(
        applications.map(async (app) => {
          const job = await Job.findOne({ _id: app.jobID });
          return {
            ...app.toObject(),
            jobDetails: job ? JSON.parse(JSON.stringify(job)) : null
          };
        })
      );
  ...
  return JSON.parse(JSON.stringify(candidatesWithDetails));
}